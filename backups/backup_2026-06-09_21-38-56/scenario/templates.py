...
def update_history():
    import traceback
    safe_print(f"\n🔵 [UPDATE_HISTORY] Called with {len(state.templates)} templates, {len(state.scenario_metadata)} scenarios")
    safe_print(f"🔵 [UPDATE_HISTORY] Call stack:")
    for line in traceback.format_stack()[-4:-1]:  # Print last 3 calls
        safe_print(f"  {line.strip()}")
    safe_print(f"🔵 [UPDATE_HISTORY] Deleting all listbox items (current size: {state.UI.history_list.size()})")
    state.UI.history_list.delete(0, tk.END)
    safe_print(f"🔵 [UPDATE_HISTORY] After delete, listbox size: {state.UI.history_list.size()}")

    # Nếu có scenario_metadata, hiển thị từng kịch bản tách riêng
    if state.scenario_metadata:
        safe_print(f"🔵 [UPDATE_HISTORY] Queue mode: displaying {len(state.scenario_metadata)} scenarios")
        for scenario_idx, metadata in enumerate(state.scenario_metadata):
            ...
            safe_print(f"🔵 [UPDATE_HISTORY] Scenario {scenario_idx + 1} has {templates_count} templates")
        ...
        safe_print(f"🔵 [UPDATE_HISTORY] Editor mode: inserting {len(state.templates)} items")
        safe_print(f"🔵 [UPDATE_HISTORY] state.templates details:")
        for i, tpl in enumerate(state.templates):
            path_str = tpl.get("path", tpl.get("key", "???"))
            safe_print(f"  {i}: {tpl['type']} - {path_str}")
    ...
    safe_print(f"🔵 [UPDATE_HISTORY] Final listbox size: {state.UI.history_list.size()}")
...
def _autosave_to_library():
    ...
    Hoặc khi ở queue mode, save toàn bộ queue."""
    from utils import safe_print
    
    # Queue mode: save scenario_metadata
    if state.scenario_metadata:
        safe_print(f"🔵 [AUTOSAVE] Queue mode: saving {len(state.scenario_metadata)} scenarios")
        try:
            from scenario.io import save_scenario_queue
            save_scenario_queue(state.scenario_metadata, state.scenario_queue)
            safe_print(f"✅ [AUTOSAVE] Queue saved successfully")
        except Exception as exc:
            safe_print(f"⚠️ [AUTOSAVE] Queue save failed: {exc}")
        return
    
    # Editor mode: save templates to library
    ...
    try:
        ...
        safe_print(f"✅ [AUTOSAVE] Saved to {state.current_library_game}/{state.current_library_stage}")
    except Exception as exc:
        safe_print(f"⚠️ [AUTOSAVE] Auto-save failed: {exc}")
...
def _resolve_selection_context(index):
    """Map line index trong history_list về context:
      ...
      ('none',)
    """
    safe_print(f"🔵 [RESOLVE] index={index}")
    if state.scenario_metadata:
        cursor = 0
        for s_idx, metadata in enumerate(state.scenario_metadata):
            # 3 dòng header: === / KỊCH BẢN N / ===
            safe_print(f"🔵 [RESOLVE] Scenario {s_idx}: cursor={cursor}")
            if cursor <= index < cursor + 3:
                safe_print(f"🔵 [RESOLVE] → scenario_header {s_idx}")
                return ("scenario_header", s_idx)
            cursor += 3
            num_items = len(metadata["templates"])
            safe_print(f"🔵 [RESOLVE]   num_items={num_items}, cursor now={cursor}")
            if num_items == 0:
                if index == cursor:
                    safe_print(f"🔵 [RESOLVE] → scenario_header {s_idx} (empty)")
                    return ("scenario_header", s_idx)
                cursor += 1
            else:
                if cursor <= index < cursor + num_items:
                    t_idx = index - cursor
                    safe_print(f"🔵 [RESOLVE] → scenario_item ({s_idx}, {t_idx})")
                    return ("scenario_item", s_idx, t_idx)
                cursor += num_items
            # blank line
            if index == cursor:
                safe_print(f"🔵 [RESOLVE] → blank line at cursor={cursor}")
                return ("none",)
            cursor += 1
        safe_print(f"🔵 [RESOLVE] → none (after scenarios)")
        return ("none",)
    if 0 <= index < len(state.templates):
        safe_print(f"🔵 [RESOLVE] → editor_item {index}")
        return ("editor_item", index)
    safe_print(f"🔵 [RESOLVE] → none (out of bounds)")
    return ("none",)
...
def delete_selected():
    ...
    # DEBUG: Print listbox content before delete
    safe_print(f"\n🔵 [DEBUG DELETE] ========== DELETE START ==========")
    safe_print(f"🔵 [DEBUG DELETE] Queue mode: {bool(state.scenario_metadata)}")
    safe_print(f"🔵 [DEBUG DELETE] state.scenario_metadata: {len(state.scenario_metadata)} items")
    safe_print(f"🔵 [DEBUG DELETE] state.templates: {len(state.templates)} items")
    safe_print(f"🔵 [DEBUG DELETE] Selected index: {selected[0]}")
    safe_print(f"🔵 [DEBUG DELETE] Listbox items count: {state.UI.history_list.size()}")
    
    ctx = _resolve_selection_context(selected[0])
    safe_print(f"🔵 [DEBUG DELETE] Resolved context: {ctx}")
    
    kind = ctx[0]
    if kind == "editor_item":
        idx = ctx[1]
        safe_print(f"🔵 [DEBUG DELETE] Deleting editor_item at index {idx}")
        safe_print(f"🔵 [DEBUG DELETE] Before: {len(state.templates)} templates")
        state.templates.pop(idx)
        safe_print(f"🔵 [DEBUG DELETE] After: {len(state.templates)} templates")
        update_history()
        _autosave_to_library()
        state.UI.status_label.config(text="🗑️ Đã xóa mục.")
    elif kind == "scenario_item":
        s_idx, t_idx = ctx[1], ctx[2]
        safe_print(f"🔵 [DEBUG DELETE] Deleting scenario_item at scenario {s_idx}, template {t_idx}")
        safe_print(f"🔵 [DEBUG DELETE] Before: {len(state.scenario_metadata[s_idx]['templates'])} templates in scenario {s_idx}")
        state.scenario_metadata[s_idx]["templates"].pop(t_idx)
        safe_print(f"🔵 [DEBUG DELETE] After: {len(state.scenario_metadata[s_idx]['templates'])} templates in scenario {s_idx}")
        update_history()
        _autosave_to_library()  # ← ADD THIS
        ...
        if not messagebox.askyesno("Xác nhận xóa", f"Xóa kịch bản '{scenario_name}' khỏi queue?"):
            return
        safe_print(f"🔵 [DEBUG DELETE] Deleting scenario at index {s_idx}")
        ...
        safe_print(f"🔵 [DEBUG DELETE] Cannot delete: kind = {kind}")
...
def _open_image_edit_dialog(tpl):
    ...
    tk.Label(content_frame, text="⏱️ Delay trước (giây):", font=("Segoe UI", 11, "bold"),
             bg="white", fg="black").pack(anchor="w", pady=(10, 3))
    delay_before_var = tk.StringVar(value=str(tpl.get("delay_before", 0)))
    tk.Entry(content_frame, textvariable=delay_before_var, font=("Segoe UI", 11), width=40).pack(anchor="w", pady=(0, 15), fill=tk.X)
    ...
    if result["ok"]:
        try:
            ...
            tpl["delay_before"] = float(fields["delay_before"].get()) if fields["delay_before"].get() else 0
