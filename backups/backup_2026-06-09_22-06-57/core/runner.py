    ...
    try:
        ...
        if state.game_hwnd:
            if not win32gui.IsWindow(state.game_hwnd):
                safe_print(f"❌ FATAL: Window handle {state.game_hwnd} is no longer valid!")
                safe_print("   Window may have been closed, minimized, or alt-tabbed away")
                safe_print("   Please select the target window again using 'Chọn Cửa Sổ Đích'")
                state.game_hwnd = None
                state.running = False
                if state.UI and hasattr(state.UI, 'status_label') and state.UI.status_label:
            ...
            # RESTORE AND PROTECT WINDOW
            if not WindowGuard.protect_window():
                safe_print("❌ Could not protect/restore window")
                state.game_hwnd = None
                state.running = False
                if state.UI and hasattr(state.UI, 'status_label') and state.UI.status_label:
                    state.UI.status_label.config(
                        text="❌ Không thể bảo vệ cửa sổ đích. Vui lòng chọn lại.",
                        fg="#ff3333"
                    )
                return "failed"
            
            safe_print("✅ Window validated and protected successfully")
        ...
        while state.running and (state.infinite_loop or loop_count < state.process_loops):
            ...
            for tpl_index, tpl in enumerate(state.templates):
                ...
                # Check window is still valid before processing each template
                if state.game_hwnd:
                    if not WindowGuard.validate_window():
                        safe_print(f"❌ FATAL: Window handle became invalid during execution!")
                        safe_print("   The game window was closed, minimized, or lost focus")
                        state.game_hwnd = None
                        state.running = False
                        if state.UI and hasattr(state.UI, 'status_label') and state.UI.status_label:
                            state.UI.status_label.config(
                                text="❌ Cửa sổ đích bị mất trong quá trình chạy",
                                fg="#ff3333"
                            )
                        break
                    
                    # PROTECT WINDOW: restore if minimized, bring to foreground
                ...
                if tpl["type"] == "image":
                    ...
                    for i in range(tpl["repeat"]):
                        ...
                        if tpl.get("is_relative", False):
                            # Relative coordinates are stored as position relative to game window top-left
                            # Need to convert back to screen coordinates by adding window offset
                            try:
                                from core.relative_capture import RelativeCoordinateCapture
                                
                                # Get current window info to get offset
                                win_info = RelativeCoordinateCapture.get_game_window_info()
                                if win_info:
                                    # Convert relative back to absolute screen coordinates
                                    click_x = win_info['client_left'] + tpl["x"]
                                    click_y = win_info['client_top'] + tpl["y"]
                                    safe_print(f"📍 Relative→Screen: ({tpl['x']}, {tpl['y']}) + offset({win_info['client_left']}, {win_info['client_top']}) = ({click_x}, {click_y})")
                                else:
                                    # If can't get window info, use direct coordinates
                                    click_x = tpl["x"]
                                    click_y = tpl["y"]
                                    safe_print(f"⚠️ No window info, using direct coords: ({click_x}, {click_y})")
                            except Exception as e:
                                safe_print(f"⚠️ Lỗi xử lý relative coordinates: {e}")
                                click_x = tpl["x"]
                                click_y = tpl["y"]
...
def smart_start(event=None):
    ...
    if not state.game_hwnd:
        msg = "⚠️ Chưa xác định cửa sổ game. Vui lòng bấm '🎯 Xác Định Cửa Sổ Đích' trước!"
        state.UI.status_label.config(text=msg, fg="#ff6600")
        safe_print(msg)
        return
    ...
    if warning:
        ...
        # Try to auto-recover by finding window by title
        if state.game_window_title:
            safe_print(f"🔍 Trying to find window by title: {state.game_window_title}")
            found_hwnd = RelativeCoordinateCapture.find_window_by_title(state.game_window_title)
            if found_hwnd:
                state.game_hwnd = found_hwnd
                safe_print(f"✅ Window found and restored: {state.game_window_title} (HWND: {found_hwnd})")
                state.UI.status_label.config(
                    text=f"✅ Cửa sổ đã được tìm lại: {state.game_window_title}",
                    fg="#00cc00"
                )
            else:
                # If still not found, ask user
                safe_print(f"❌ Window not found by title: {state.game_window_title}")
                import tkinter as tk
                from tkinter import messagebox
                result = messagebox.askyesno(
                    "⚠️ Cảnh báo",
                    f"{warning}\n\nBạn có muốn:\n"
                    "- 'Có' để chọn cửa sổ khác\n"
                    "- 'Không' để hủy bỏ"
                )
                
                if result:
                    from autoclick_gui import set_target_window
                    set_target_window()
                return
        else:
            # No window title saved, ask user to select
            import tkinter as tk
            from tkinter import messagebox
            result = messagebox.askyesno(
                "⚠️ Cảnh báo",
